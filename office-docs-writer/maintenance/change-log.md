...
## 2026-04-30（版记与结构验证优化）

### 变更内容
- 新增 `guidelines/footer-imprint.md`：公文版记结构规范（324行）
- 新增 `guidelines/historical-docx-reference.md`：历史 DOCX 检索与复刻协议（299行）
- 更新 `validators/style-checklist.md`：新增第十二节"结构性校验"
- 更新 `SKILL.md`：追加版记路由提示
- 更新 `AGENTS.md`：追加步骤3.5和版记硬约束
- 更新 `agent-config.md`：追加步骤3.5和禁止行为
- 更新 `document-types/work-summary/checklist.md`：追加版记校验小节
- 更新 `maintenance/backlog.md`：追加6个低优先级边缘情况

### 触发事件
`C:\SkillsSetup\handoff-word格式调整记录.md` — DOCX 生成失败（版记写成普通段落而非表格）
